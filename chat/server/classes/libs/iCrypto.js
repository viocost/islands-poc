...
class iCrypto {
    ...
    streamCryptorDecrypt(cryptorID = iCrypto.pRequired("streamCryptorEncrypt"),
        ...
        let cryptor = self.get(cryptorID);
        if (encoding === undefined || encoding === "raw"){
            return cryptor.decrypt(blob).buffer
        } else {
            throw new Error("NOT IMPLEMENTED");
        }
    ...
    //TODO implement PEM key validator
    static isPemKeyValid(keyData){

    }

    /**
     * Converts private key in PEM format into raw base64 string
     * Does NOT do any PEM validation. Just strips last and first lines
     * and removes all the \n characters
     *
     *
     * @param keyData
     */
    static pemToBase64(keyData){
        return keyData.trim().split(/\r?\n/).slice(1, -1).join("");
    }
