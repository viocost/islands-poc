...
class iCrypto {
    constructor(settings) {
        ...
        self.rsa = {
            createKeyPair: (...args) => {
                return self.generateRSAKeyPair(...args);
            },
            asyncCreateKeyPair: (...args) => {
                return self.asyncGenerateRSAKeyPair(...args);
            },
            encrypt: (...args) => {
                return self.publicKeyEncrypt(...args);
            },
            decrypt: (...args) => {
                return self.privateKeyDecrypt(...args);
            },
            sign: (...args) => {
                return self.privateKeySign(...args);
            },
            verify: (...args) => {
                return self.publicKeyVerify(...args);
            },
            setKey: (...args) => {
                return self.setRSAKey(...args);
            },
            getSettings: () => {
                return "RSA";
            }
        ...
        self.aes = {
            ...
            keySize: 32,
            createKey: (...args) => {
                return self.createSYMKey(...args);
            },
            encrypt: (...args) => {
                return self.AESEncrypt(...args);
            },
            decrypt: (...args) => {
                return self.AESDecrypt(...args);
            },
            setKey: (...args) => {
                return self.setSYMKey(...args);
            },
            getSettings: () => {
                return "AES";
            }
        ...
        self.chacha = {
            init: (...args) => {
                return self.initStreamCryptor(...args);
            },
            encrypt: (...args) => {
                return self.streamCryptorEncrypt(...args);
            },
            decrypt: (...args) => {
                return self.streamCryptorDecrypt(...args);
            },
            getSettings: () => {
                return "ChaCha";
            }
     ...
     */
    setSYMKey(nameToSave = iCrypto.pRequired("setSYMKey"), key = iCrypto.pRequired("setSYMKey")) {
     ...
     */
    AESDecrypt(target = iCrypto.pRequired("AESDecrypt"), key = iCrypto.pRequired("AESDecrypt"), nameToSave = iCrypto.pRequired("AESDecrypt"), dehexify = false, mode = "CBC", encoding) {
        let self = this;
        let cipherWOIV;
        if (!self.aes.modes.includes(mode.toUpperCase())) {
            throw "AESencrypt: Invalid AES mode";
     ...
     */
    hashFileWorker(file = iCrypto.pRequired("fileHashWorker file"), nameToSave = iCrypto.pRequired("fileHashWorker nameToSave"), algorithm = "sha256") {
        return new Promise((resolve, reject) => {
            let self = this;
            if (Worker === undefined) {
     ...
     */
    initStreamCryptor(nameToSave = iCrypto.pRequired("initStreamEncryptor"), key = iCrypto.pRequired("initStreamEncryptor"), isEncryptionMode = true, algorithm = "chacha20") {
        let self = this;
    ...
    streamCryptorEncrypt(cryptorID = iCrypto.pRequired("streamCryptorEncrypt"), blob = iCrypto.pRequired("streamCryptorEncrypt"), encoding = "raw") {
        let self = this;
        let input;
    ...
    streamCryptorDecrypt(cryptorID = iCrypto.pRequired("streamCryptorEncrypt"), blob = iCrypto.pRequired("streamCryptorEncrypt"), encoding = "raw") {
        let self = this;
        let cryptor = self.get(cryptorID);
     ...
     * @param algorithm
     */
    hash(target = iCrypto.pRequired("hash"), nameToSave = iCrypto.pRequired("hash"), algorithm = "sha256") {
        let self = this;
    ...
    createHash(nameToSave = iCrypto.pRequired("createHash"), algorithm = "sha256") {
    ...
    digestHash(target = iCrypto.pRequired("digestHash"), nameToSave = iCrypto.pRequired("digestHash"), hexify = true) {
        let self = this;
        let hRes = self.get(target);
        let res = hexify ? sjcl.codec.hex.fromBits(hRes.finalize()) : sjcl.codec.arrayBuffer.fromBits(hRes.finalize());
        this.set(nameToSave, res);
        return self;
    }

    asyncGenerateRSAKeyPair(nameToSave = iCrypto.pRequired("asyncGenerateRSAKeyPair"), length = 2048) {
        return new Promise((resolve, reject) => {
            let self = this;
     ...
     * @returns {iCrypto}
     */
    publicFromPrivate(target = iCrypto.pRequired("publicFromPrivate"), nameToSave = iCrypto.pRequired("publicFromPrivate")) {
     ...
     * @returns {iCrypto}
     */
    setRSAKey(nameToSave = iCrypto.pRequired("setRSAPublicKey"), keyData = iCrypto.pRequired("setRSAPublicKey"), type = iCrypto.pRequired("setRSAPublicKey")) {
     ...
     * @returns {iCrypto}
     */
    getPublicKeyFingerprint(target = iCrypto.pRequired("getPublicKeyFingerpint"), nameToSave = iCrypto.pRequired("getPublicKeyFingerpint"), hashAlgorithm = "sha256") {
    ...
    publicKeyEncrypt(target = iCrypto.pRequired("publicKeyEncrypt"), key = iCrypto.pRequired("publicKeyEncrypt"), nameToSave = iCrypto.pRequired("publicKeyEncrypt"), encoding) {
     ...
     * @private
     */
    _encodeBlob(blob = iCrypto.pRequired("_encodeBlob"), encoding = iCrypto.pRequired("_encodeBlob")) {
        let self = this;
    ...
    _decodeBlob(blob = iCrypto.pRequired("_encodeBlob"), encoding = iCrypto.pRequired("_encodeBlob")) {
    ...
    privateKeyDecrypt(target = iCrypto.pRequired("privateKeyDecrypt"), key = iCrypto.pRequired("privateKeyDecrypt"), nameToSave = iCrypto.pRequired("privateKeyDecrypt"), encoding) {
    ...
    publicKeyVerify(target = iCrypto.pRequired("publicKeyVerify"), signature = iCrypto.pRequired("publicKeyVerify"), key = iCrypto.pRequired("publicKeyVerify"), nameToSave = iCrypto.pRequired("publicKeyVerify"), dehexifySignRequired = true) {
        key = this.validateExtractRSAKey(this.get(key), "public");
        let publicKey = forge.pki.publicKeyFromPem(key);
     ...
     */
    validateExtractRSAKey(key = iCrypto.pRequired("validateAndExtractRSAKey"), keyType = iCrypto.pRequired("validateAndExtractRSAKey")) {
    ...
    pemToBase64(target = iCrypto.pRequired("pemToBase64"), nameToSave = iCrypto.pRequired("pemToBase64"), keyType = iCrypto.pRequired("pemToBase64")) {
     ...
     */
    decompress(target = iCrypto.pRequired("decompress"), nameToSave = iCrypto.pRequired("decompress")) {
    ...
    encode(target = iCrypto.pRequired("encode"), encoding = iCrypto.pRequired("encode"), nameToSave = iCrypto.pRequired("encode")) {
        let self = this;
        self.set(nameToSave, self._encodeBlob(this.get(target), encoding));
        return this;
    }

    base64Encode(name = iCrypto.pRequired("base64Encode"), nameToSave = iCrypto.pRequired("base64Encode"), stringify = false) {
        let target = stringify ? JSON.stringify(this.get(name)) : this.get(name);
        this.set(nameToSave, iCrypto.base64Encode(target));
        return this;
    }

    base64Decode(name = iCrypto.pRequired("base64decode"), nameToSave = iCrypto.pRequired("base64decode"), jsonParse = false) {
    ...
    /**/
    bytesToHex(name = iCrypto.pRequired("bytesToHex"), nameToSave = iCrypto.pRequired("bytesToHex"), stringify = false) {
        let target = stringify ? JSON.stringify(this.get(name)) : this.get(name);
        this.set(nameToSave, iCrypto.hexEncode(target));
        return this;
    }

    hexToBytes(name = iCrypto.pRequired("hexToBytes"), nameToSave = iCrypto.pRequired("hexToBytes"), jsonParse = false) {
    ...
    stringifyJSON(name = iCrypto.pRequired("stringify"), nameToSave = iCrypto.pRequired("stringify")) {
    ...
    parseJSON(name = iCrypto.pRequired("stringify"), nameToSave = iCrypto.pRequired("stringify")) {
        let target = this.get(name);
    ...
    encodeBlobLength(target = iCrypto.pRequired("encodeBlobLength"), targetLength = iCrypto.pRequired("encodeBlobLength"), paddingChar = iCrypto.pRequired("encodeBlobLength"), nameToSave = iCrypto.pRequired("encodeBlobLength")) {
